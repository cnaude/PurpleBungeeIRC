...
public class MultichatChatListener
        implements Listener {

    final private PurpleIRC plugin;

    public MultichatChatListener(PurpleIRC plugin) {
        this.plugin = plugin;
    }

    @EventHandler
    public void receievePluginMessage(PluginMessageEvent event) throws IOException {
        if (!event.getTag().equalsIgnoreCase("MultiChat")) {
            return;
        }
        ...
        MultiChatMessage cm = new MultiChatMessage(in);
